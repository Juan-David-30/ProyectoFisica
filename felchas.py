...
class Ball:
    def __init__(self, x, y, vx, vy, mass, color, radius):
        ...
        self.arrow_length = 40  # Length of arrow
        self.angle = math.pi / 4  # Default angle of arrow
    ...
    def draw_arrow(self, screen):
        arrow_end_x = self.x + self.arrow_length * math.cos(self.angle)
        arrow_end_y = self.y + self.arrow_length * math.sin(self.angle)
        pygame.draw.line(screen, BLACK, (self.x, self.y), (arrow_end_x, arrow_end_y), 2)
        pygame.draw.circle(screen, BLACK, (int(arrow_end_x), int(arrow_end_y)), 5)

# Function to draw ball
def draw_ball(ball, screen):
    pygame.draw.circle(screen, ball.color, (int(ball.x), int(ball.y)), ball.radius)
    ball.draw_arrow(screen)
...
def handle_events(ball1, ball2, simulation_started):
        ...
        if event.type == pygame.QUIT:
            ...
            if event.button == 1:
                if ball1.rect.collidepoint(event.pos):
                    ball1.dragging = True
                    ball1.drag_offset_x = event.pos[0] - (ball1.x + ball1.arrow_length * math.cos(ball1.angle))
                    ball1.drag_offset_y = event.pos[1] - (ball1.y + ball1.arrow_length * math.sin(ball1.angle))
                elif ball2.rect.collidepoint(event.pos):
                    ball2.dragging = True
                    ball2.drag_offset_x = event.pos[0] - (ball2.x + ball2.arrow_length * math.cos(ball2.angle))
                    ball2.drag_offset_y = event.pos[1] - (ball2.y + ball2.arrow_length * math.sin(ball2.angle))
                elif not simulation_started and start_button_rect.collidepoint(event.pos):
                    return True
            ...
            if event.button == 1:
                ...
                if simulation_started:
                    # Calculate velocities based on arrow direction
                    ball1.vx = ball1.arrow_length * math.cos(ball1.angle) / 10
                    ball1.vy = ball1.arrow_length * math.sin(ball1.angle) / 10
                    ball2.vx = ball2.arrow_length * math.cos(ball2.angle) / 10
                    ball2.vy = ball2.arrow_length * math.sin(ball2.angle) / 10
        elif event.type == pygame.MOUSEMOTION:
            if ball1.dragging:
                dx = event.pos[0] - ball1.x - ball1.drag_offset_x
                dy = event.pos[1] - ball1.y - ball1.drag_offset_y
                ball1.angle = math.atan2(dy, dx)
            elif ball2.dragging:
                dx = event.pos[0] - ball2.x - ball2.drag_offset_x
                dy = event.pos[1] - ball2.y - ball2.drag_offset_y
                ball2.angle = math.atan2(dy, dx)
...
while running:
    ...
    # Handle events
    running = handle_events(ball1, ball2, simulation_started)
